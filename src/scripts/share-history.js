...
export function normalizeSnapshot(value) {
	...
	const book = normalizeBook(value.book),
		mermaid = validateDiagram(value.mermaid, { legacy: true }).source
	const generatedAt =
...
export function encodeShare(value, url) {
	const compressed = LZString.compressToEncodedURIComponent(
		JSON.stringify(normalizeSnapshot(value)),
	)
	if (compressed.length > 16000) throw new Error('Map is too large to share')
	const target = new URL(url)
	target.search = ''
	target.hash = ''
	target.searchParams.set('graph', compressed)
	return target.href
}
...
export function loadHistory(storage) {
	...
	try {
		...
		for (const entry of parsed.slice(0, 30)) {
			try {
				const safe = normalizeSnapshot(entry)
				if (!list.some((e) => historyKey(e.book) === historyKey(safe.book)))
					list.push(safe)
			} catch {
				corrupted = true
			}
...
export function saveHistory(list, storage) {
	try {
		...
		storage.setItem(
			HISTORY_STORAGE_KEY,
			JSON.stringify(list.slice(0, 30).map(normalizeSnapshot)),
		)
		return true
	} catch {
		return false
...
export function historyKey(book) {
	...
	return safe.id || [safe.title, safe.authors.join(','), safe.year].join('|')
}
