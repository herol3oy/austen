...
document.addEventListener('astro:page-load', () => {
	...
	function renderGlobalStatus() {
		if (
			state.error &&
			(state.error.scope === 'share' ||
				(state.error.scope === 'history' && !el.historyList))
		) {
	...
	function renderSearch() {
		...
		el.searchInput.placeholder = state.book
			? 'Find another book'
	...
	function renderSelectedBook() {
		...
		generateBtn.textContent = state.generateLoading
			? 'Generating\u2026'
			: 'Generate diagram'
		...
		if (existing) {
			...
			link.href = existing.publishedUrl
			link.className = 'published-offer'
			link.textContent = 'Open the published map'
			actions.appendChild(link)
			generateBtn.textContent = 'Generate another map'
		}
		...
		if (state.error && state.error.scope === 'generate') {
			...
			setStatus(
				el.generateStatus,
				'Asking the backend for a diagram\u2026',
				null,
			)
		} else {
	...
	function renderActions() {
		...
		if (state.downloadStatus) {
			setStatus(
				el.downloadStatus,
				state.downloadStatus.text,
				state.downloadStatus.tone,
			)
		} else {
	...
	function isEditorDirty() {
		return (
			state.editorOpen && sanitizeMermaid(state.editorDraft) !== state.graph
		)
	}
	...
	function renderEditor() {
		...
		if (state.editorValidation === 'checking') {
			setStatus(
				el.editorStatus,
				'Checking syntax and updating the preview\u2026',
				null,
			)
	...
	function renderHistory() {
		...
		if (state.error && state.error.scope === 'history') {
			...
			setStatus(
				el.historyStatus,
				'Diagrams you generate will be saved here.',
				null,
			)
		} else {
  ...
  `
	...
	const generator = el.searchInput
		? createGenerator({
				getState: () => state,
				setState,
				initializeWorkspace,
	...
	function handleSearchInput(event) {
		generator?.handleSearchInput(event)
	}
	function selectBook(book) {
		generator?.selectBook(book)
	}
	...
	async function initializeWorkspace(
		{ book, mermaid: source, generatedAt = null, svg, canonicalUrl = null },
		{ remember = false } = {},
	) {
		if (disposed) return
		const loadToken = ++workspaceLoadToken
		const normalizedBook = normalizeBook(book)
		const graph = validateDiagram(source, { legacy: true }).source
		...
		state = {
			...
			generatedAt,
			canonicalUrl,
			originalGraph: canonicalUrl ? graph : null,
			error: null,
			generateLoading: false,
			downloadLoading: false,
		}
	...
	function initializeDiagramPanzoom() {
		...
		const handleKeydown = (event) => {
			if (!diagramPanzoom || event.altKey || event.ctrlKey || event.metaKey)
				return
	...
	function handleEditorCancel() {
		...
		if (
			isEditorDirty() &&
			!window.confirm('Discard your unsaved diagram edits?')
		)
			return
	...
	function buildShareUrl() {
		if (state.canonicalUrl && state.graph === state.originalGraph)
		...
		return encodeShare(
			...
			new URL(`${import.meta.env.BASE_URL}generate/`, location.origin),
		)
	}
	...
	async function restoreFromShareUrl() {
		...
		try {
			await initializeWorkspace(decodeShare(compressed))
			return true
		} catch {
			setState({
				error: {
					scope: 'share',
					message:
						"That share link looks corrupted or unsupported, so it wasn't loaded.",
				},
			})
			return false
		}
	...
	function saveHistory(list) {
		...
		setState({
			error: {
				scope: 'history',
				message: "Couldn't save history locally (storage may be full).",
			},
		})
		return false
	}
	...
	function handleDeleteHistory(entry) {
		...
		historyUndoTimer = setTimeout(() => {
			...
			if (undoHadFocus) {
				requestAnimationFrame(() =>
					el.historyHeading.focus({ preventScroll: true }),
				)
			}
	...
	function addHistoryEntry(book, mermaidStr, generatedAt) {
		...
		const nextHistory = [
			{ book, mermaid: mermaidStr, generatedAt },
			...withoutDuplicate,
	...
	async function restoreFromHistory(entry) {
		...
		try {
			await initializeWorkspace(entry)
			if (disposed) return
			history.replaceState(history.state, '', buildShareUrl())
		} catch {
			setState({
				error: {
					scope: 'history',
					message: 'This saved map could not be rendered.',
				},
			})
		}
	}
	...
	function buildDiagramFilename(extension) {
		const title =
			state.book && typeof state.book.title === 'string' ? state.book.title : ''
		const slug = title
			.normalize('NFKD')
			.replace(/[\u0300-\u036f]/g, '')
			.toLowerCase()
	...
	async function createDiagramPng() {
		const { markup, width, height } = getNormalizedDiagramSvg(
			PNG_EXPORT_BACKGROUND,
		)
	...
	async function init() {
		...
		el.downloadSvgBtn.addEventListener('click', () =>
			handleDiagramDownload('svg'),
		)
		...
		const initial = JSON.parse(
			document.getElementById('workspace-data').textContent || 'null',
		)
		...
		setState({
			...
			error: corrupted
				? {
						scope: 'history',
						message: "Your saved history couldn't be read, so it was reset.",
					}
				: state.error,
		})
	}
	...
	document.addEventListener(
		...
		() => {
			...
			cancelEditorPreviewWork()
			clearHistoryUndoTimer()
			for (const timer of Object.values(flashTimers)) clearTimeout(timer)
			destroyDiagramPanzoom()
		},
		{ once: true },
	)
	...
	init().catch(() =>
		setState({
			error: {
				scope: 'share',
				message: 'The workspace could not be initialized.',
			},
		}),
	)
})
