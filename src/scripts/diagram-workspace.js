...
function renderSelectedBook() {
	...
	yearP.textContent = state.book.year ? String(state.book.year) : 'Year unknown'
...
function handleEditorCancel() {
	if (!state.editorOpen) return
	if (isEditorDirty() && !window.confirm('Discard your unsaved diagram edits?'))
		return
...
function buildShareUrl() {
	...
	return encodeShare(
		{ book: state.book, mermaid: state.graph, generatedAt: state.generatedAt },
