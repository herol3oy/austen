...
export function authorCollections(entries: CatalogEntry[]): AuthorCollection[] {
	...
	for (const [name, books] of authors) {
		...
		if (existing !== undefined)
			throw new Error(
				`Author slug collision: ${JSON.stringify(existing)} and ${JSON.stringify(name)} both use ${slug}`,
			)
		slugs.set(slug, name)
		collections.push({
			name,
...
export function collectionsForBook(
	book: Pick<Book, 'authors'>,
...
export function relatedMaps(
	...
	for (const author of collectionsForBook(book, collections)) {
		for (const other of author.books)
			if (other.id !== book.id) matches.set(other.id, other)
	}
