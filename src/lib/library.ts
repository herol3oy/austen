...
export interface Book {
  ...
  sourceUrl: string; sourceAnchor: string | null; ebookUrl: string; metadataWarnings: string[];
}
...
export async function loadLibrary(root = process.cwd()) {
  ...
  for (const book of catalog.books as Book[]) {
    ...
    entries.push({ ...book, eligible: !unavailable, publishedUrl: map ? `${import.meta.env.BASE_URL}books/${book.slug}/` : null,
