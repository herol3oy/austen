...
const record = (book, source) => ({
	status: 'imported',
	repository: standardEbooksRepository(book),
	branch: 'master',
	sourceSha: sha,
...
test('cover manifests accept only local Standard Ebooks assets', async () => {
	...
	const manifest = {
		...
		books: { [book.id]: record(book, source) },
	}
	validateCovers(manifest)
	assert.equal(
		savedCoverSlug(book, manifest),
		'jane-austen-pride-and-prejudice',
	)
	assert.equal(
		standardEbooksRepository(book),
		'jane-austen_pride-and-prejudice',
	)
	assert.equal(
		standardEbooksRepository({ id: `${'a'.repeat(70)}/${'b'.repeat(70)}` })
			.length,
		100,
	)
		...
		validateCovers({
			...
			books: {
				[book.id]: {
					...
					assetPath: 'https://github.com/cover.webp',
				},
			},
		}),
	)
...
test('imports only published catalog books, stores local assets, and skips stored covers on rerun', async (t) => {
	...
	await writeJson(resolve(root, 'data/published.json'), {
		schemaVersion: 1,
		books: [{ bookId: book.id }, { bookId: second.id }],
	})
	const source = image()
	let _requests = 0
	const fetchImpl = async (url, options) => {
		...
		if (path === '/orgs/standardebooks/repos')
			return Response.json([{ name: standardEbooksRepository(book) }])
		if (path.endsWith('/contents/images/cover.jpg'))
			return Response.json({ sha, size: source.length })
	...
	assert.deepEqual(
		{
			...
			missingRepository: result.missingRepository,
			failed: result.failed,
		},
		{ downloaded: 1, missingRepository: 1, failed: 0 },
	)
...
test('migrates valid version 2 covers to Astro assets without redownloading them', async (t) => {
	...
	await writeJson(resolve(root, 'data/catalog/covers.json'), {
		...
		books: {
			[book.id]: {
				...
				branch: 'master',
				sourceSha: sha,
				sourcePath: `data/covers/${book.slug}/cover.jpg`,
				displayPath: `covers/${book.slug}/cover.jpg`,
				sourceHash: hash(source),
				displayHash: hash(source),
				checkedAt: generatedAt,
			},
		},
	})
	...
	await writeFile(legacySource, source)
	await writeFile(legacyDisplay, source)
	const result = await syncCovers({
		root,
		token: 'test-token',
		fetchImpl: async (url) => {
	...
	assert.equal(
		(await readFile(resolve(root, coverAssetPath(book)))).equals(source),
		true,
	)
	await assert.rejects(access(legacySource))
	await assert.rejects(access(legacyDisplay))
})
