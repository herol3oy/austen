...
test('publication is automatic, permits optional review, and detects source/SVG tampering; regeneration preserves the pointer', async (t) => {
	...
	assert.equal(automatic.reviewer, undefined)
	assert.throws(
		() =>
			validatePublicationManifest({
				schemaVersion: 1,
				books: [{ ...automatic, checks: 'source render svg' }],
			}),
		/publication checks/,
	)
	assert.deepEqual(
	...
	await assert.rejects(
		publishMap({ root, id: book.id, revision: 'v1', reviewed: true }),
	...
	await publishMap({
		...
		notes: 'Fixture only; not a public approval.',
	})
	...
	await writeFile(path.replace('.json', '.svg'), svg.replace('Jane', 'Janet'))
	await assert.rejects(verifyArtifact(root, book, 'v1'), /hash/)
...
test('SVG policy rejects active content, event handlers, external references and invalid dimensions', () => {
	for (const bad of [
		svg.replace('<g>', '<g onload="alert(1)">'),
		svg.replace('<g>', '<script>alert(1)</script><g>'),
		svg.replace('<g>', '<foreignObject></foreignObject><g>'),
		svg.replace('<g>', '<g style="fill:url(https://example.org/a)">'),
		svg.replace('<g>', '<g style="fill:u&#114;l(https://example.org/a)">'),
		svg.replace('300 100', '0 0'),
	])
		assert.throws(() => validateSvg(bad))
...
test('local corrections create a new attributed revision and leave the parent intact', async (t) => {
	...
	const before = await readFile(path, 'utf8'),
		source = resolve(root, 'correction.mmd')
	await writeFile(source, graph.replace('Sister of', 'Sisters'))
	const revised = await reviseMap({
		root,
		id: book.id,
		from: 'v1',
		source,
	...
	assert.equal(revised.revision, 'v2')
	assert.equal(revised.origin.type, 'local_edit')
	assert.equal(revised.origin.parentRevision, 'v1')
	assert.equal(revised.outcome, 'valid')
