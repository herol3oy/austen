...
const request = (
	book = { title: 'Book', authors: [], publishYear: 1813 },
	extra = {},
) =>
...
test('Worker preserves successful response, accepts legacy year, returns UNKNOWN distinctly and redacts upstream errors', async (t) => {
	...
	assert.equal((await response.json()).mermaid, graph)
	...
	assert.equal((await response.json()).code, 'UNKNOWN')
	globalThis.fetch.mock.mockImplementation(
		async () => new Response('sensitive details', { status: 401 }),
	)
	response = await worker.fetch(request(), env)
...
test('Worker validates authors and size, rejects origins, honors rate limit and kill switch', async (t) => {
	...
	assert.equal(
		(await worker.fetch(request({ title: 'Book', authors: 'bad' }), env))
			.status,
		400,
	)
	assert.equal(
		(await worker.fetch(request({ title: 'x'.repeat(5000), authors: [] }), env))
			.status,
		400,
	)
	assert.equal(
	...
	assert.equal(
		(await worker.fetch(request(), { ...env, GENERATION_ENABLED: 'false' }))
			.status,
		503,
	)
	const response = await worker.fetch(request(), {
		...env,
		GENERATION_LIMITER: { limit: async () => ({ success: false }) },
	})
