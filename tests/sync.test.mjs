...
import { readJson, withLock, writeJson } from '../scripts/files.mjs'
import { runBatch } from '../scripts/generate-maps.mjs'
import { countAttempts, readBookRecords } from '../scripts/map-records.mjs'
import { loadPublishedMaps, publishMap } from '../scripts/publish-map.mjs'
import { parseSyncArgs, syncMaps } from '../scripts/sync-maps.mjs'
...
test('sync defaults to a five-pass dry run; dry run has no rendering or writes', async (t) => {
	assert.equal(parseSyncArgs([]).passes, 5)
	assert.equal(parseSyncArgs([]).dryRun, true)
	assert.equal(parseSyncArgs(['--run']).dryRun, false)
	assert.equal(parseSyncArgs(['--run', '--dry-run']).dryRun, true)
	const { root } = await fixtureRoot(t)
	const result = await syncMaps({
		root,
		provider: noProvider,
		renderer: noProvider,
		log: () => {},
	...
	assert.equal(result.maximumRequests, 15)
...
test('whole catalog publishes more than 50 books across categories without a selection or manual review', async (t) => {
	...
	catalog.books = Array.from({ length: 55 }, (_, i) => ({
		...book,
		id: `fixture-author/book-${i}`,
		slug: `fixture-author-book-${i}`,
		title: `Book ${i}`,
		category: i % 2 ? 'Poetry' : 'Nonfiction',
	...
	await writeJson(resolve(root, 'data/catalog/selection.json'), {
		ignored: true,
	})
	let calls = 0
	const result = await syncMaps({
	...
	assert.equal(result.published, 55)
	assert.equal(result.actualUsage.total_tokens, 8250)
	const { maps } = await loadPublishedMaps(root, catalog)
	assert.equal(maps.length, 55)
	assert.ok(
		maps.every(
			(m) =>
				m.review.mode === 'automatic' &&
				!m.review.reviewer &&
				m.revision.outcome === 'valid',
		),
	)
	const before = await readFile(resolve(root, 'data/published.json'), 'utf8')
	const resumed = await syncMaps({
		...options(root),
		provider: noProvider,
		renderer: noProvider,
	})
	assert.equal(resumed.reused, 55)
...
test('each pass visits pending books once, skips UNKNOWN and success, and publishes a later successful attempt', async (t) => {
	...
	const result = await syncMaps({
		...
		provider: async (metadata) => {
			...
			const count = (counts.get(metadata.title) || 0) + 1
			counts.set(metadata.title, count)
			if (metadata.title === catalog.books[1].title) return unknown()
			if (metadata.title === catalog.books[0].title && count < 3)
				return invalid()
			return success()
		},
	})
...
test('old attempts count toward five, and repeated runs cannot reset exhausted books', async (t) => {
	...
	assert.equal(countAttempts(await readBookRecords(root, book)), 5)
	const result = await syncMaps({
		...options(root),
		ids: [book.id],
		provider: noProvider,
...
test('Mermaid parse errors trigger another generation pass rather than a browser setup stop', async (t) => {
	...
	const result = await syncMaps({
		...
		provider: async () => success(),
		renderer: async (source) => {
			if (source !== PREFLIGHT_GRAPH && ++renders === 1)
				throw new RenderError('invalid_graph', 'Parse error')
			return svg
		},
	})
...
test('missing credentials and failed browser preflight stop before any provider request or revision', async (t) => {
	...
	let calls = 0
	await assert.rejects(
		syncMaps({ ...options(root), apiKey: '', renderer: noProvider }),
		/DEEPSEEK_API_KEY/,
	)
	await assert.rejects(
		syncMaps({
		...
		/Browser preflight failed.*Chrome/,
	)
...
test('Retry-After persists across restarts and applies to other books, with short waits', async (t) => {
	...
	const result = await syncMaps({
		...
		provider: async () => {
			assert.ok(delays.reduce((a, b) => a + b, 0) > 64000)
			return success()
		},
	})
	assert.ok(delays.every((ms) => ms <= 30000))
	assert.equal(result.published, 1)
})
...
test('timeouts and 5xx share the five-attempt ceiling', async (t) => {
	...
	const result = await syncMaps({
		...
		provider: async () => {
			calls++
			throw new ProviderError(calls % 2 ? 'timeout' : 'provider_error', {
				retryable: true,
				status: calls % 2 ? 0 : 503,
			})
		},
	})
...
test('legacy manual publications remain byte-for-byte unchanged and saved valid revisions publish for free', async (t) => {
	...
	const published = await syncMaps({
		...options(root),
		ids: [book.id],
		renderer: noProvider,
		provider: noProvider,
	...
	const manifestPath = resolve(root, 'data/published.json'),
		manifest = await readJson(manifestPath)
	delete manifest.books[0].mode
	delete manifest.books[0].publishedAt
...
test('an outcome of valid alone cannot bypass artifact verification', async (t) => {
	...
	await assert.rejects(
		syncMaps({ ...options(root), ids: [book.id], provider: noProvider }),
		/Revision is not valid/,
	)
	assert.equal(
		(await readJson(resolve(root, 'data/published.json'))).books.length,
		0,
	)
})
