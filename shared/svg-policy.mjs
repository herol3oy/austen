...
export function checkSvgElements(nodes) {
	if (nodes[0]?.name.toLowerCase() !== 'svg')
		throw new Error('Expected SVG root')
	for (const node of nodes) {
		if (!elements.has(node.name.toLowerCase()))
			throw new Error(`Unsupported SVG element: ${node.name}`)
		for (const [key, value] of Object.entries(node.attributes)) {
			if (
				/^on/i.test(key) ||
				/^(?:href|xlink:href|src)$/i.test(key) ||
				/[\\]|@import|expression\s*\(|javascript:|data:|https?:|\/\//i.test(
					value.replace(
						/http:\/\/www\.w3\.org\/2000\/svg|http:\/\/www\.w3\.org\/1999\/xlink/g,
						'',
					),
				)
			)
	...
	const viewBox = (nodes[0].attributes.viewBox || '')
		.trim()
		.split(/[\s,]+/)
		.map(Number)
	if (
...
function checkCss(value = '') {
	if (
		/[\\]|@(?!keyframes\b)|expression\s*\(|javascript:|data:|https?:|\/\*/i.test(
			value,
		)
	) {
	...
	for (const match of value.matchAll(/url\s*\(([^)]*)\)/gi))
		if (!/^['"]?#[a-z0-9_-]+['"]?$/i.test(match[1].trim()))
