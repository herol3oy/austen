...
export function decompressShare(input, limit = 40000) {
	...
	const first = bits(2)
	if (first === 2) return ''
	if (first > 1) throw new Error('Invalid compressed share')
	...
	for (;;) {
		...
		size += entry.length
		if (size > limit) throw new Error('Decoded share exceeds size limit')
		output.push(entry)
