...
export function parseDiagram(raw, { legacy = false } = {}) {
	...
	if (!source || source.length > MAX_GRAPH_LENGTH)
		throw new Error('Graph is empty or exceeds 12,000 characters')
	if (
		/[<>]/.test(source.replace(/-->/g, '')) ||
		// biome-ignore lint/suspicious/noControlCharactersInRegex: Reject control characters in untrusted diagram source while allowing tabs and newlines.
		/[{};`\\]|%%|(?:https?:|javascript:|data:)|[\u0000-\u0008\u000b-\u001f]/i.test(
			source,
		)
	)
		throw new Error('Graph contains unsupported markup or directives')
	const lines = source
		.split(/\r?\n/)
		.map((l) => l.trim())
		.filter(Boolean)
	...
	if (
		legacy
			? !/^graph (?:LR|RL|TD|TB|BT)$/.test(direction)
			: direction !== 'graph LR'
	)
		throw new Error('Use graph LR')
	const node =
		'([A-Za-z][A-Za-z0-9_]{0,63})(?:\\[("[^"\\[\\]<>|]{1,100}"|[^"\\[\\]<>|]{1,100})\\])?'
	const declaration = new RegExp(`^${node}$`, 'u')
	const edge = new RegExp(
		`^${node}\\s*-->\\s*(?:\\|([^|<>]{1,80})\\|\\s*)?${node}$`,
		'u',
	)
	...
	function addNode(id, label) {
		...
		if (label !== undefined)
			nodes.set(id, { id, label: label.replace(/^"|"$/g, '').trim() || id })
	...
	for (const line of lines) {
		...
		if (match) {
			...
			if (!match)
				throw new Error(
					'Use one character declaration or directional relationship per line',
				)
			addNode(match[1], match[2])
		}
	}
	if (
		...
		throw new Error('Graph must have 2–20 characters and 1–40 relationships')
