...
export const standardEbooksRepository = (book) =>
	book.id.replaceAll('/', '_').slice(0, 100)
...
export function validateCovers(manifest) {
	...
	for (const [id, record] of Object.entries(manifest.books)) {
		...
		if (record.status === 'imported') {
			if (
				!/^[-a-z0-9_]{1,100}$/.test(record.repository) ||
				record.branch !== 'master' ||
				!gitSha.test(record.sourceSha) ||
				...
				throw new Error(`Invalid cover record: ${id}`)
		} else if (nullable.some((key) => record[key] !== null))
			throw new Error(`Invalid cover record: ${id}`)
	}
	return manifest
}
...
export function savedCoverSlug(book, manifest) {
	...
	return record?.status === 'imported' &&
		record.assetPath === coverAssetPath(book)
		? book.slug
		: undefined
}
