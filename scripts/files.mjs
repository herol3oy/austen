...
export const isMain = (url) =>
	process.argv[1] && fileURLToPath(url) === resolve(process.argv[1])
export async function withLock(root, task) {
	...
	try {
		...
		try {
			await writeFile(recovery, owner, { flag: 'wx' })
		} catch {
			throw new Error(
				'Another maintenance process is recovering the lock. Retry when it finishes.',
			)
		}
		try {
			...
			if (previous) {
				if (
					...
					previous.pid <= 0 ||
					(previous.hostname && previous.hostname !== hostname())
				)
					throw new Error(
						'Maintenance lock has an unknown owner; inspect data/.maintenance.lock.',
					)
				let alive = true
				try {
				...
				if (alive)
					throw new Error(
						`Maintenance is already running (PID ${previous.pid}).`,
					)
				await unlink(path)
			}
			try {
				...
				if (error.code === 'EEXIST')
					throw new Error('Another maintenance process acquired the lock.')
				throw error
			}
		} finally {
