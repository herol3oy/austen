...
import {
	...
	coverAssetPath,
	savedCoverSlug,
	standardEbooksRepository,
	validateCovers,
} from '../shared/covers.mjs'
import {
	atomicWrite,
	hash,
	isMain,
...
const imported = (book, repository, sourceSha, sourceHash) => ({
	repository,
	branch: 'master',
	sourceSha,
	assetPath: coverAssetPath(book),
	sourceHash,
})
...
async function githubToken(explicitToken) {
	...
	try {
		return (
			(
				await execute('gh', ['auth', 'token'], { timeout: 5000 })
			).stdout.trim() || null
		)
	} catch {
		return null
...
async function upgradeManifest(root, rawManifest, catalog, dryRun) {
	if (
		...
		!rawManifest.books ||
		typeof rawManifest.books !== 'object'
	)
		return null
	...
	for (const [id, record] of Object.entries(rawManifest.books)) {
		...
		const checkedAt = Number.isFinite(Date.parse(record?.checkedAt))
			? record.checkedAt
		...
		if (
			...
			upgraded.books[id] = {
				status: ['missing_repository', 'missing_cover', 'failed'].includes(
					record?.status,
				)
					? record.status
					: 'failed',
		...
		try {
			...
			if (!validJpeg(source) || hash(source) !== record.sourceHash)
				throw new Error('legacy source is invalid')
			if (!dryRun) {
				...
				await removeIfUnchanged(
					oldDisplay,
					typeof record.displayHash === 'string'
						? record.displayHash
						: record.sourceHash,
				)
			}
...
export async function syncCovers({
	...
	if (!Array.isArray(published?.books))
		throw new Error('Invalid published map index')
	const publishedIds = new Set(published.books.map((entry) => entry.bookId))
	const books = (
		id ? catalog.books.filter((book) => book.id === id) : catalog.books
	).filter((book) => publishedIds.has(book.id))
	if (id && !catalog.books.some((book) => book.id === id))
		throw new Error('Unknown catalog ID')
	if (id && !books.length)
		throw new Error('That catalog book has no published map')
	...
	if (manifest) {
		...
		try {
			manifest = validateCovers(rawManifest)
		} catch {
			manifest = structuredClone(emptyManifest)
		}
	}
	...
	const request = async (path, { allow404 = false } = {}) => {
		for (let attempt = 0; attempt < 3; attempt++) {
			...
			try {
				...
				response = await fetchImpl(`${apiRoot}${path}`, {
					...
					headers: {
						...
						'User-Agent': 'austen-cover-importer',
					},
				})
			} catch (error) {
				if (attempt === 2)
			...
			if (apiError(response))
				throw fatalGithubError(
					`GitHub authentication or rate limit error (HTTP ${response.status}): ${body.slice(0, 200)}`,
				)
			if (response.status < 500 || attempt === 2)
				throw new Error(
					`GitHub returned HTTP ${response.status}: ${body.slice(0, 200)}`,
				)
			await wait(
				Math.min(
					60000,
					Math.max(retryDelay(response, now), 1000 * 2 ** attempt),
				),
			)
		}
	}
	...
	for (let page = 1; ; page++) {
		const response = await request(
			`/orgs/standardebooks/repos?type=public&per_page=100&page=${page}`,
		)
		const entries = await responseJson(response, 'repository list')
		if (!Array.isArray(entries))
			throw new Error('Invalid GitHub repository list')
		for (const entry of entries)
			if (typeof entry?.name === 'string') repositories.add(entry.name)
	...
	for (const book of books) {
		...
		if (
			...
			(await storedCoverIsValid(root, book, previous))
		) {
			cached++
			continue
		}
		const repository = standardEbooksRepository(book)
		const record = (status, details = missing()) => {
		...
		try {
			const content = await request(
				`/repos/standardebooks/${encodeURIComponent(repository)}/contents/images/cover.jpg?ref=master`,
				{ allow404: true },
			)
			if (content.status === 404) {
				record('missing_cover')
				missingCover++
				await save()
				continue
			}
			...
			if (
				...
				throw new Error('Unexpected cover metadata')
			if (
				previous?.status === 'imported' &&
				previous.sourceSha === entry.sha &&
				(await storedCoverIsValid(root, book, previous))
			) {
				cached++
				continue
			...
			const blob = await request(
				`/repos/standardebooks/${encodeURIComponent(repository)}/git/blobs/${entry.sha}`,
			)
			const data = await responseJson(blob, 'cover blob')
			if (data?.encoding !== 'base64' || typeof data.content !== 'string')
				throw new Error('Unexpected cover blob')
			const source = Buffer.from(data.content.replace(/\s/g, ''), 'base64')
			if (source.length !== entry.size)
				throw new Error('Cover blob size does not match metadata')
			...
			record('imported', imported(book, repository, entry.sha, sourceHash))
			downloaded++
			await save()
		} catch (error) {
			...
			if (
				...
				record('failed')
			log(`Cover import failed for ${book.id}: ${error.message}`)
			await save()
		}
	}
	const summary = {
