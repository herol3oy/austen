...
export function validatePublicationManifest(published) {
	...
	for (const pointer of published.books) {
		...
		if (
			...
			pointer.checks.some((c) => typeof c !== 'string')
		)
			throw new Error('Invalid publication checks')
		seen.add(pointer.bookId)
		if (pointer.mode === 'automatic') {
			if (
				!Number.isFinite(Date.parse(pointer.publishedAt)) ||
				!['source', 'render', 'svg'].every((c) =>
					pointer.checks?.includes(c),
				) ||
				pointer.reviewer ||
				pointer.reviewedAt
			)
			...
			if (
				...
				![
					...
					'directions',
					'readability',
				].every((c) => pointer.checks?.includes(c))
			)
...
export async function loadPublishedMaps(root, catalog) {
	const published = validatePublicationManifest(
		await readJson(resolve(root, 'data/published.json'), {
			schemaVersion: 1,
			books: [],
		}),
	)
...
export async function createPublisher(root, catalog) {
	...
	const pointers = new Map(
		published.books.map((pointer) => [pointer.bookId, pointer]),
	)
	return {
		pointers,
		maps,
		async publish(book, revision, { reviewed = false, reviewer, notes } = {}) {
			if (reviewed && (!cleanText(reviewer) || !cleanText(notes)))
				throw new Error('Manual review requires --reviewer NAME --notes TEXT')
			const artifact = await verifyArtifact(root, book, revision)
			const existing = pointers.get(book.id)
			if (existing?.revision === revision) {
				if (
					existing.revisionHash !== artifact.revisionHash ||
					existing.svgHash !== artifact.revision.validation.svgHash
				)
					throw new Error('Published artifact changed after publication')
			...
			const pointer = {
				...
				svgHash: artifact.revision.validation.svgHash,
				publishedAt: now,
				...(reviewed
					? {
							mode: 'manual',
							reviewedAt: now,
							reviewer: cleanText(reviewer),
					...
					: { mode: 'automatic', checks: ['source', 'render', 'svg'] }),
			}
			const books = [
				...published.books.filter((p) => p.bookId !== book.id),
				pointer,
			]
...
export async function publishMap({
	...
	return (await createPublisher(root, catalog)).publish(book, revision, {
		reviewer,
		notes,
		reviewed,
	})
...
if (isMain(import.meta.url)) {
	...
	withLock(ROOT, () => publishMap(values))
		.then((p) => console.log(`Published ${p.bookId} ${p.revision}`))
		.catch((error) => {
