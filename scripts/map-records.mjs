...
export async function readBookRecords(root, book) {
	...
	for (const name of names
		.filter((n) => /^v[1-9]\d*\.json$/.test(n))
		.sort((a, b) => Number(a.slice(1, -5)) - Number(b.slice(1, -5)))) {
		const path = resolve(dir, name),
			revision = await readJson(path)
		if (
			revision.schemaVersion !== 1 ||
			revision.bookId !== book.id ||
			revision.revision !== name.slice(0, -5) ||
			!Array.isArray(revision.attempts)
		)
			throw new Error(`Invalid saved revision: ${path}`)
...
export function countAttempts(records) {
	...
	return records
		.filter((r) => r.revision.origin?.type !== 'local_edit')
		.reduce((sum, r) => sum + r.revision.attempts.length, 0)
}
...
export function nextAction(records) {
	...
	const record = records.at(-1),
		outcome = record?.revision.outcome
	if (['candidate', 'render_failed'].includes(outcome))
		return { action: 'validate', record }
	if (outcome === 'unknown_work') return { action: 'unknown', record }
	if (countAttempts(records) >= MAX_BOOK_ATTEMPTS)
